...
public class MongoDBClient extends CacheClientBase implements ICacheClient,
		INoSQLClient {

	protected static DBObject toDbObject(Object obj) throws ExceptionCommonBase {
		if (obj == null)
			return null;
		try {
					...
					if (value != null) {
						...
						if (Utils.stringIsNullOrEmpty(name))
							throw new ExceptionCommonBase(
									ErrorCodeBase.NotImplementCode);
	...
	private static Object get_Id(Object obj) throws ExceptionCommonBase {
		...
		if (primrayKey.length == 0) {
			throw new ExceptionCommonBase(ErrorCodeBase.ParameterError,
					"db obj primary key is null ,class = "
	...
	public MongoDBClient(String dbname) throws ExceptionCommonBase {
	...
	public synchronized DB getDB() throws ExceptionCommonBase {
		if (db == null) {
			Mongo m = getMongo();
			db = m.getDB(this.dbname);
			if (!Utils.stringIsNullOrEmpty(this.getUserName())
					&& !Utils.stringIsNullOrEmpty(this.getUserPassword()))
				db.authenticate(this.getUserName(), this.getUserPassword()
						.toCharArray());
		}
		return db;
	}

	public synchronized MongoClient getMongo() throws ExceptionCommonBase {

		if (mongo == null) {
			try {
				if (this.getPoolSize() > 0)
					System.setProperty("MONGO.POOLSIZE",
							String.valueOf(this.getPoolSize()));
	...
	private boolean save(String table, DBObject obj) throws ExceptionCommonBase {
	...
	@Override
	public <T extends Object> T getOne(T query) throws ExceptionCommonBase {
		if (query == null)
			throw new ExceptionCommonBase(ErrorCodeBase.ParameterError,
					"query is null");
	...
	public <T> List<T> getList(T query, LinkedHashMap<String, Boolean> orderBy,
			int skip, int count) throws ExceptionCommonBase {
		List<T> list = new ArrayList<T>();
		DBCollection table = getCollection(getTableName(query));
		DBCursor cur = table.find(toQueryDbObject(query))
				.sort(toQueryDbObject(orderBy)).skip(skip).limit(count);
		while (cur.hasNext()) {
